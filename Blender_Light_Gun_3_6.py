...
def update_preview_light_position(scene, prev_empty=None):
    ...
    preview_light = scene.objects.get('Preview Light')
    if settings.draw_lights_active:
        # In Draw Lights mode, do not apply any 3D positioning here;
        # let mouse ray update it later
        # print("update_preview_light_position() ending early")
        return
    camera = scene.camera
...
class ToggleDrawLightsOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # ✅ Force preview light update if preview mode is active
        if settings.preview_mode != 'NONE':
            bpy.ops.object.preview_light_update()
            ...
            try:
                ...
                if location and normal and object is not None:
                    ...
                    if is_2d_object:
                        ...
                        # Calculate the directio1n of the ray itself (from start point to hit point)
                        ray_direction = (location - view_location).normalized()

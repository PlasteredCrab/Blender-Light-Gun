...
def update_preview_light():
    settings = bpy.context.scene.raycast_light_tool_settings
    preview_light = bpy.context.scene.objects.get('Preview Light')
    ...
    if settings.preview_mode and preview_light:
        ...
        if settings.light_type == 'POINT':
            ...
            print("Setting AREA light shape to:", settings.light_area_shape)
...
class RAYCAST_PT_edit_light(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if light.type == 'POINT':
            ...
            layout.row().label(text="Advanced Properties")
            layout.prop(light, "diffuse_factor", text="Diffuse")
            layout.prop(light, "specular_factor", text="Specular")
            layout.prop(light, "volume_factor", text="Volume")
...
def kelvin_to_rgb(color_temperature):
    ...
    if color_temperature <= 1500:
        ...
        green = green * (1 - 2 * (diff / 1000.0) ** 2)
        blue = blue * (1 - 2 * (diff / 1000.0) ** 2)
...
class FOV_UPDATE_OT_timer_operator(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        wm = context.window_manager
        self._timer = wm.event_timer_add(1, window=context.window) # setting up the timer
        wm.modal_handler_add(self) # adding the operator to modal handlers
        return {'RUNNING_MODAL'}

    def modal(self, context, event):
        wm = context.window_manager
        camera_data = context.object.data
        
        if not camera_data.show_fov_lines:
            self.cancel(context)
            wm.is_fov_timer_running = False
            return {'FINISHED'}

        if event.type == 'TIMER':
            #print("[INFO] Timer event triggered!")
            update_fov_lines(context.object.data, context)
...
class FOV_TOGGLE_OT_start_stop(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if camera.show_fov_lines:
            # If currently showing FOV lines, stop the timer
            camera.show_fov_lines = False
...
def update_fov_lines(self, context):
    ...
    # Check if 'Show FOV Lines' is checked
    if camera_data.show_fov_lines:
...
class RAYCAST_PT_edit_camera(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.operator("fov_toggle.start_stop", text="Toggle FOV Lines")
        if obj.data.show_fov_lines:
...
class RAYCAST_PT_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if settings.light_type == 'POINT':
            ...
            layout.row().label(text="Advanced Properties")
            layout.prop(settings, "light_diffuse", text="Diffuse")
            layout.prop(settings, "light_specular", text="Specular")
            layout.prop(settings, "light_volume", text="Volume")
            ...
            layout.prop(settings, "light_specular", text="Specular")
            layout.prop(settings, "light_volume", text="Volume")
            layout.prop(settings, "light_spot_blend", text="Blend")
            layout.prop(settings, "light_show_cone", text="Show Cone")
...
class RaycastLightToolSettings(bpy.types.PropertyGroup):
    ...
    light_placement_mode: bpy.props.EnumProperty(
        ...
        update=RAYCAST_OT_update_light_preview.update,
    )
...
def unregister():
    ...
    bpy.utils.unregister_class(LIGHT_OT_switch_to_temperature)
    bpy.utils.unregister_class(LIGHT_OT_switch_to_rgb)
